...
//tft lcd spi ili9488 3.5" 480x320
void Lcd_Init(void) {
    ...
    Lcd_Write_Data(0x28 | 0x40); //0x28 | 0x80 - ровно, относительно надписи, 0x88 | 0x40 - влево, 0x08 - вправо, 0x28 | 0x40 - 180 относительно надписи "Отладочная плата"
...
void LCD_SetScrollArea(uint16_t top, uint16_t scroll, uint16_t bottom)
{
    ...
    Lcd_Write_Data(top & 0xFF);
    Lcd_Write_Data(scroll >> 8);
    Lcd_Write_Data(scroll & 0xFF);
    Lcd_Write_Data(bottom >> 8);
    Lcd_Write_Data(bottom & 0xFF);
}

void LCD_Scroll(uint16_t position)
{
    Lcd_Write_Com(0x37);
    Lcd_Write_Data(position >> 8);
    Lcd_Write_Data(position & 0xFF);
}
...
const uint8_t* utf8_rus_lookup(const char **text)
{
    ...
    if (c < 0x80) {
        ...
        return Char_font1[c - 32];
    }
    uint8_t c2 = (uint8_t)(*text)[1];
    //  А-Я 
    if (c == 0xD0 && c2 >= 0x90 && c2 <= 0xAF) {
        (*text) += 2;
        return Char_font2[c2 - 0x90];  // 0–31
    }
    //  а-п 
    if (c == 0xD0 && c2 >= 0xB0 && c2 <= 0xBF) {
        (*text) += 2;
        return Char_font2[32 + (c2 - 0xB0)];  // 32–47
    }
    // р-я 
    if (c == 0xD1 && c2 >= 0x80 && c2 <= 0x8F) {
        (*text) += 2;
        return Char_font2[32 + 16 + (c2 - 0x80)];  // 48–63
    }
    (*text)++;
    return NULL;
}

void drawGlyph(uint16_t x, uint16_t y, const uint8_t *bitmap, uint16_t color, uint16_t bg, uint8_t scale)
{
    for (uint8_t col = 0; col < 5; col++)       // 5 колонок в символе
    {
        uint8_t line = bitmap[col];
        for (uint8_t row = 0; row < 8; row++)   // 8 строк в колонке
        {
            uint16_t draw_color = (line & (1 << row)) ? color : bg;
            ClearMassDMA(scale, scale, x + col * scale, y + row * scale, draw_color);
        }
    }
}
...
//     while (*text) {
//         const uint8_t *bitmap = utf8_rus_lookup(&text);
//         if (!bitmap) continue;

//         drawGlyph(cursor_x, y, bitmap, color, bg, scale);
//         cursor_x += 6 * scale;
//     }
// }
...
void drawText(uint16_t x, uint16_t y, const char *text, uint16_t color, uint16_t bg, uint8_t scale)
{
    ...
    while (*text) 
    {
        if (*text == '\n') 
        {
            cursor_x = x;                  // начало строки
            cursor_y += 10 * scale;        // сдвиг вниз, 10 = высота символа + межстрочный интервал
            text++;
            continue;
        }
        ...
        drawGlyph(cursor_x, cursor_y, bitmap, color, bg, scale);
        cursor_x += 6 * scale;             // ширина символа + 1
    }
}
...
void PrintMassDMA_Fast(const uint16_t *mas,uint16_t length,uint16_t width,uint16_t d_l,uint16_t d_w)
{
    ...
    static uint8_t bufA[320 * 3];
    static uint8_t bufB[320 * 3];

    uint8_t *active = bufA;
    uint8_t *next   = bufB;

    const uint16_t *row_ptr = mas;

    // Первая строка
    uint8_t *p = active;
    for (uint16_t x = 0; x < width; x++)
    {
        uint16_t pixel = *row_ptr++;
        // RGB565 - RGB666 
        *p++ = (pixel >> 8) & 0xF8;  // R
        *p++ = (pixel >> 3) & 0xFC;  // G
        *p++ = (pixel << 3);         // B
    }
    HAL_DMA_Start(&hdma_ch0, active,(void*)&hspi0.Instance->TXDATA,width * 3 - 1);
    // Остальные строки
    for (uint16_t y = 1; y < length; y++)
    {
        ...
        // ждём завершения DMA
        HAL_DMA_Wait(&hdma_ch0, DMA_TIMEOUT_DEFAULT);
        // запускаем следующий DMA
        HAL_DMA_Start(&hdma_ch0, next,(void*)&hspi0.Instance->TXDATA, width * 3 - 1);
        // меняем буферы
        uint8_t *tmp = active;
        active = next;
        next = tmp;
...
static inline uint16_t overlay_pixel(const uint16_t *bg, uint16_t bg_w,const uint16_t *fg, uint16_t fg_w, uint16_t fg_h,uint16_t fg_x, uint16_t fg_y,uint16_t x, uint16_t y) {
    //попали ли в область маленького изображения
    if (x >= fg_x && x < fg_x + fg_h && y >= fg_y && y < fg_y + fg_w) {
        uint16_t fx = x - fg_x;
        uint16_t fy = y - fg_y;
        uint16_t p = fg[fx * fg_w + fy];
...
//ширина длина, ширина длина, высота широта, широта высота
...
{
    Lcd_select();
    Address_set(d_w, d_l, bg_w - 1 + d_w, bg_h - 1 + d_l);
    HAL_GPIO_WritePin(GPIO_0, RS, 1);

    if (!(hspi0.Instance->ENABLE & SPI_ENABLE_M))
        __HAL_SPI_ENABLE(&hspi0);

    for (uint16_t y = 0; y < bg_h; y++) {
        for (uint16_t x = 0; x < bg_w; x++) {

            uint16_t color = overlay_pixel(bg, bg_w,fg, fg_w, fg_h,fg_x, fg_y,x, y);
            DMA_BUF[x * 3 + 2] = (color >> 8) & 0xF8;
            DMA_BUF[x * 3 + 0] = (color >> 3) & 0xFC;
            DMA_BUF[x * 3 + 1] = (color << 3);
        }

        HAL_DMA_Start(&hdma_ch0,(void*)DMA_BUF,(void*)&hspi0.Instance->TXDATA, bg_w * 3 - 1);
        HAL_DMA_Wait(&hdma_ch0, DMA_TIMEOUT_DEFAULT);
    }
...
uint16_t GetBackgroundColor(uint32_t status){ //переход в другой цвет со временем
    uint8_t r = 0, g = 0, b = 0;
...
void PrintMassBW_RLE_DMA(const uint8_t *rle, uint32_t rle_len)
{
    Lcd_select();
    ...
    static uint8_t bufA[480 * 3];
    static uint8_t bufB[480 * 3];
    ...
    while (rle_idx < rle_len && y < 320)
    {
        uint8_t run = rle[rle_idx++]; //сколько байтов подряд имеют одинаковый val
        uint8_t val = rle[rle_idx++]; //байт с 8 битами пикселей (например 0x03 0xFF 3 байта с 8 белыми пиксялими в каждои - 24 белых пикселя)
        ...
        for (uint8_t r = 0; r < run; r++)
        {
            ...
            {
                uint8_t bit = (val >> i) & 1;
                uint8_t color = bit ? 0xFF : 0x00; // 1 - белый, 0 - черный
                ...
                *p++ = color;
                ...
                {
                    HAL_DMA_Wait(&hdma_ch0, DMA_TIMEOUT_DEFAULT);
                    HAL_DMA_Start(&hdma_ch0, active, (void*)&hspi0.Instance->TXDATA, 480 * 3 - 1);
                    uint8_t *tmp = active;
                    active = next;
                    next = tmp;
                    p = active;
...
void PrintFramesBW_RLE_DMA(const FrameRLE_t *frames, int frame_count){
    for (int i = 0; i < frame_count; i++) PrintMassBW_RLE_DMA(frames[i].data, frames[i].len);
}