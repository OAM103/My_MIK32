...
void ClearMassDMA(uint16_t length, uint16_t width, uint16_t d_l, uint16_t d_w, uint16_t color) {
    ...
    // Подготовка буфера DMA: заполняем всю строку одинаковым цветом
    for (uint16_t i = 0; i < width; i++) {
        dma_buffer[i*3 + 0] = (color >> 8) & 0xF8;  // зеленый
        dma_buffer[i*3 + 1] = (color >> 3) & 0xFC;  // синий
        dma_buffer[i*3 + 2] = (color << 3);         // красный
    }
...
void draw_line(int x_start, int y_start, int x_end, int y_end, uint16_t color){
    ...
    {
        ClearMass(1,1,steep ? y : x, steep ? x : y, color);
        error -= dy;
        if (error < 0)
        {
            y += ystep;
