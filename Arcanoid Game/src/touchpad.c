...
void process_platform_touch(const uint16_t PLATFORM_COLOR, const uint16_t BG_COLOR) {
    ...
    ClearMassDMA(right_platform.height, right_platform.width,right_platform.y - right_platform.height/2,right_platform.x - right_platform.width/2,PLATFORM_COLOR);
    ...
    if (x_raw > 0 && y_raw < 2032) {
        ...
        if (abs(coord_x - right_platform.x) <= PLATFORM_TOUCH_RADIUS-30 &&
            ...
            // стираем старую платформу
            ClearMassDMA(right_platform.height, right_platform.width,
