...
void drawChar(uint16_t x, uint16_t y, char ch, uint16_t color, uint16_t bg, uint8_t scale) {
    ...
    for (uint8_t col = 0; col < 5; col++) {
        uint8_t line = reverse_bits(bitmap[col]); // переворачиваем биты
        for (uint8_t row = 0; row < 7; row++) {
            uint8_t pixel_on = (line & (1 << row)) ? 1 : 0;
            for (uint8_t sx = 0; sx < scale; sx++) {
                for (uint8_t sy = 0; sy < scale; sy++) {
                    // Поворот 90° влево: x' = x - row*scale - sy, y' = y + col*scale + sx
                    uint16_t px = x - (row * scale + sy);
                    uint16_t py = y + (col * scale + sx);
                    drawPixel(px, py, pixel_on ? color : bg);
                }
            }
        }
    }
...
void drawText(uint16_t x, uint16_t y, const char *text, uint16_t color, uint16_t bg, uint8_t scale) {
    while (*text) {
        ...
        y += 6 * scale; // сдвиг вниз для следующего символа
        text++;
    }
}
