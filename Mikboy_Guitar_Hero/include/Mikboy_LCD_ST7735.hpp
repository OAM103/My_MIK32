...
namespace ST7735
{
    ...
    constexpr uint16_t SIZE_BUF = 520;
    ...
    private:
        ...
        HAL_StatusTypeDef write_char(uint16_t x_start, uint16_t y_start, const uint8_t word, uint16_t *mas, uint8_t scale) const;
        ...
        /// @brief Рисуем спрайт поверх градиентного фона 
        /// @param x начало по столбцам
        /// @param y начало по строкам
        /// @param src массив пикселей картинки
        /// @param w ширина картинки
        /// @param h высота картинки
        /// @param from_color какой цвет надо заменить
        /// @param to_color на что этот цвет заменить
        /// @param grad_top начало градиента фона
        /// @param grad_bottom конец градиента фона
        /// @return HAL_OK или состояние ошибки
        HAL_StatusTypeDef print_mas_bg_gradient(uint16_t x, uint16_t y, const uint16_t* src, uint16_t w, uint16_t h, uint16_t from_color, uint16_t to_color, uint16_t grad_top, uint16_t grad_bottom) const;
        ...
        /// @param grad_bottom конец градиента фона
        /// @return HAL_OK или состояние ошибки
        HAL_StatusTypeDef print_mas_gradient(uint16_t x, uint16_t y, const uint16_t* src, uint16_t w, uint16_t h, uint16_t from_color, uint16_t to_color, uint16_t grad_top, uint16_t grad_bottom) const;

        /// @brief Рисуем градиентный фон
        /// @param x начало по столбцам
        /// @param y начало по строкам
        /// @param w ширина картинки
        /// @param h высота картинки
        /// @param color_top цвет в верхней части экрана
        /// @param color_bottom цвет в нижней части экрана
        /// @return HAL_OK или состояние ошибки
        HAL_StatusTypeDef gradient(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color_top, uint16_t color_bottom) const;

        /// @brief Прямоугольник с вертикальным градиентом относительно всего экрана
        /// @param x_start X верхний левый угол
        /// @param y_start Y верхний левый угол
        /// @param x_end X нижний правый угол
        /// @param y_end Y нижний правый угол
        /// @param color_top цвет в верхней части экрана
        /// @param color_bottom цвет в нижней части экрана
        /// @return HAL_OK или состояние ошибки
        HAL_StatusTypeDef draw_fill_rect_gradient( uint16_t x_start, uint16_t y_start, uint16_t x_end, uint16_t y_end, uint16_t color_top, uint16_t color_bottom) const;
        ...
        /// @return HAL_OK или состояние ошибки
        HAL_StatusTypeDef print_word(uint16_t x_start,
                                   uint16_t y_start,
                                   const char *word,
                                   uint32_t color,
                                   uint8_t font_size) const;
    };
}
#endif