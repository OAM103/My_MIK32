...
int32_t predictor = 932;
int32_t adpcm_index = 0;

MusicTrack tracks[1] = {
    {"MAGICSOUND", MagicSound, MAGICSOUND_LEN, MAGICSOUND_FIRST, MAGICSOUND_INDEX},
};
...
const int index_table[16] = {-1,-1,-1,-1,2,4,6,8,-1,-1,-1,-1,2,4,6,8}; // как изменить ADPCM index после каждого нибла
...
void fill_dac_buf(uint16_t *buf)
{
    ...
    {
        uint8_t byte = track->data[music_pos / 2]; // 1 байт = 2 сэмпла
        uint8_t nibble = (music_pos % 2 == 0) ? (byte & 0x0F) : (byte >> 4); //нибл = 4 бита (знак | размер_изменений0 | размер_изменений1 | размер_изменений2)

        int16_t sample = adpcm_decode_nibble(nibble);

        int32_t scaled = ((int32_t)sample * volume) / 200; // регулировка громкости
        scaled = scaled >> 4; //деление на 16, чтобы не клиппило
        int32_t out = DAC_MID + scaled; // превращаем сигнал -2048..2047 в 0..4095
        ...
        music_pos++;
        if (music_pos / 2 >= track->len && ((touch_active == 1 && play_sound == 0) || (touch_active == 0 && play_sound == 1))) {
            play_sound = 0;
            music_pos = 0;
        }
    }
