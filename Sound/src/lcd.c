...
void drawCircle(int x0, int y0, int r, uint16_t color)
{
    ...
    while (y >= x)
    {
        // рисуем горизонтальные линии (4 штуки)
        
        // верх-низ
        ClearMassDMA_Fast(1, 2*y + 1, y0 + x, x0 - y, color);
        ClearMassDMA_Fast(1, 2*y + 1, y0 - x, x0 - y, color);

        // боковые
        ClearMassDMA_Fast(1, 2*x + 1, y0 + y, x0 - x, color);
        ClearMassDMA_Fast(1, 2*x + 1, y0 - y, x0 - x, color);
...
void gradientCircle(int x0, int y0, int r_circle, int r_grad, uint16_t colorCenter,  uint16_t colorBg) //Рисует круг с градиентом
{
    int r_circle2 = r_circle * r_circle; // квадраты радиусов (работаем в квадрате расстояния, что бы не считать корень)
    int r_grad2   = r_grad * r_grad;

    // Распаковка цветов
    uint8_t r1 = (colorCenter >> 8) & 0xF8;
    uint8_t g1 = (colorCenter >> 3) & 0xFC;
    uint8_t b1 = (colorCenter << 3);

    uint8_t r2 = (colorBg >> 8) & 0xF8;
    uint8_t g2 = (colorBg >> 3) & 0xFC;
    uint8_t b2 = (colorBg << 3);

    // Предрасчёт (чтобы убрать деление из цикла) (t = (dist^2 - r_circle^2) / (r_grad^2 - r_circle^2))
    uint32_t inv = (255 << 8) / (r_grad2 - r_circle2 + 1);

    static uint8_t line_buf[480 * 3]; // собираем одну строку

    for (int y = -r_grad; y <= r_grad; y++) // идём по строкам круга
    {
        int yy = y * y; // для формулы расстояния dist^2 = x^2 + y^2

        int x_left  = -r_grad; // поиск границ строки круга
        int x_right =  r_grad;
        ...
        {
            int dist2 = x*x + yy; // расстояни до центра круга

            if (dist2 > r_grad2) // вне круга
            {
                if (started) break;
                else continue;
            }

            if (!started) // фиксируем левую границу
            {
                x_left = x;
                started = 1;
            }
            ...
            {
                // градиент без деления
                uint32_t t = ((dist2 - r_circle2) * inv) >> 8;

                if (t > 255) t = 255;

                R = r1 + ((r2 - r1) * t >> 8); // интерполяция линейный переход
                G = g1 + ((g2 - g1) * t >> 8);
                B = b1 + ((b2 - b1) * t >> 8);
            }
            *p++ = R;
        ...
        int width = (p - line_buf) / 3; // столько пикселей реально в строке
        int draw_y = y0 + y;            // координата строки

        if (draw_y < 0 || draw_y >= 480) continue; // область, гдк может быть круг

        Lcd_select();
        Address_set(x0 + x_left, draw_y, x0 + x_left + width - 1, draw_y);
        HAL_GPIO_WritePin(GPIO_0, RS, 1);
...
void PrintMass(const uint16_t *mas, uint16_t length, uint16_t width, uint16_t d_l, uint16_t d_w ){
  ...
  for(m=0;m<length;m++){//перебирает строки изображения
    for(i=0;i<width;i++)//перебирает пиксели в строке
    {
      ...
      colout[0] = ((*(mas+i*length - 1 + m)>>8) & 0xF8);//извленает красный компонент из 16-битного знач. цвета, сдвигает на 8 бит и маскирует, чтобы получить 5 старших бит(& 0xF8 - 8-битное знач. умножаем на 255/31)
      colout[1] = ((*(mas+i*length - 1 + m)>>3) & 0xFC);//извлекает зеленый компонент сдвигает и маскирует(& 0xFC - 255/63)
      colout[2] = ((*(mas+i*length - 1 + m))<<3);//извлекает синий бит(<<3 - умножаем на 255/31)
...
void PrintMassDMA(const uint16_t *mas, uint16_t length, uint16_t width, uint16_t d_l, uint16_t d_w)
{
    ...
    for (uint16_t y = 0; y < length; y++)
    {
        for (uint16_t x = 0; x < width; x++)
        {
            ...
            // uint16_t pixel = *(mas + x * length - 1 + y);
...
void PrintMassBW_RLE_DMA(const uint8_t *rle, uint32_t rle_len) { 
    ...
    uint16_t y = 0; 
    uint8_t *p = active; 
    while (rle_idx < rle_len && y < height) { 
        uint8_t run = rle[rle_idx++]; 
        ...
        for (uint8_t r = 0; r < run; r++) { 
            for (int i = 7; i >= 0; i--) { 
                ...
                *p++ = color & 0xFF; 
                x++; 
                if (x >= width) { 
                    if (y == 0) HAL_DMA_Start(&hdma_ch0, active, (void*)&hspi0.Instance->TXDATA, width * 2 - 1); 
                    else { 
                        HAL_DMA_Wait(&hdma_ch0, DMA_TIMEOUT_DEFAULT); 
                        HAL_DMA_Start(&hdma_ch0, active, (void*)&hspi0.Instance->TXDATA, width * 2 - 1); 
                    } 
                    uint8_t *tmp = active; 
                    active = next; next = tmp; 
                    p = active; 
                    x = 0; 
                    y++; 
                    if (y >= height) break; 
